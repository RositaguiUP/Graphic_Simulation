...
var legend = g.append("g").attr(
  "transform",
  "translate(" + (width - 10) + "," + (height - 125) + ")"
);

// Axis
var xAxisGroup = g.append("g").attr("class", "x axis").attr(
  "transform",
  "translate(0, " + height + ")"
);
  ...
  .then((data) => {
    ...
    formData = data.map((year) => {
      return year["countries"]
        .filter((country) => {
          var dataExists = country.income !== null && country.life_exp !== null;
          return dataExists;
        })
        .map((country) => {
